...
- For simulator work, build from this firmware repo unless the change belongs in `inkmod-simulator` itself.
...
- EPUB cache lives under `.inkmod/epub_<hash>/`.
...
- Clear the relevant `.inkmod/epub_<hash>/` cache when testing EPUB parser, layout, image, or binary cache format changes that may otherwise reuse stale output.
