...
inkMOD is firmware for the Xteink X4 and X3, built with PlatformIO targeting the ESP32-C3 microcontroller.
...
- `src/InkMODSettings.h` (`SETTINGS`): user preferences and behavior flags
- `src/InkMODState.h` (`APP_STATE`): runtime/session state such as current book and sleep context

Most SD-card persistence lives under `/.inkmod/`. See file inventory in
`docs/data-cache.md`; for binary cache formats, see `docs/file-formats.md`.

## Networking architecture

Network file transfer is controlled by `src/activities/network/InkMODWebServerActivity.h` and served by `src/network/InkMODWebServer.h`.
