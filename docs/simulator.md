...
inkMOD can run in the [InkMOD simulator](https://github.com/alpzoloto-sudo/rus_mod), which renders the e-ink display in an SDL2 window. Use it for quick sanity checks without flashing firmware every time.
...
On first open of an EPUB, an **Indexing...** popup appears while the section cache is built in `.inkmod/`.

If rendering looks stale after a code change, delete `./fs_/.inkmod/` to clear simulator caches.
