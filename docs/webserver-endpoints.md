...
available while inkMOD is in File Transfer or Calibre Wireless mode.
...
Examples use `inkmod.local`. If mDNS does not resolve on your network, use
...
```bash
curl http://inkmod.local/api/status
```
...
```bash
curl "http://inkmod.local/api/files?path=/Books"
```
...
```bash
curl -OJ "http://inkmod.local/download?path=/Books/MyBook.epub"
```
...
```bash
curl -X POST -F "file=@mybook.epub" "http://inkmod.local/upload?path=/Books"
```
...
```bash
curl -X POST -d "name=NewFolder&path=/" http://inkmod.local/mkdir
```
...
```bash
curl -X POST -d "path=/Books/old.epub&name=new.epub" http://inkmod.local/rename
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub&dest=/Read" http://inkmod.local/move
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub" http://inkmod.local/delete
curl -X POST -d 'paths=["/Books/old.epub","/OldFolder"]' http://inkmod.local/delete
```
...
```bash
curl http://inkmod.local/api/settings
```
...
curl -X POST \
  ...
  http://inkmod.local/api/settings
```
...
```bash
curl http://inkmod.local/api/fonts
```
...
curl -X POST \
  ...
  http://inkmod.local/api/fonts/upload
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"family":"Literata"}' \
  http://inkmod.local/api/fonts/delete
```
...
```bash
curl http://inkmod.local/api/opds
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"index":0}' \
  http://inkmod.local/api/opds/delete
```
...
```bash
curl http://inkmod.local/api/wifi
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"index":0}' \
  http://inkmod.local/api/wifi/delete
```
...
- `inkmod.local` is advertised with mDNS when available.
- `/api/status` returns `"mode": "STA"` and RSSI in dBm.

### Access Point Mode (AP)

- Device creates an open hotspot named `InkMOD-Reader`.
