...
This guide explains how to use inkMOD's built-in web server for file
...
| **Calibre Wireless** | You want to receive books from the InkMOD Calibre plugin workflow. |
...
- The mDNS fallback URL, usually `http://inkmod.local/`
...
3. Open the URL shown on the reader. `http://inkmod.local/` is preferred
...
InkMOD Calibre plugin or other clients that speak the documented WebSocket
upload protocol.
...
2. Prefer `inkmod.local` when available, but keep the displayed IP address as a fallback.
