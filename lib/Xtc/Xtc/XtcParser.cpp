...
XtcError XtcParser::loadPageStreaming(
    uint32_t pageIndex, std::function<void(const uint8_t* data, size_t size, size_t offset)> callback,
    size_t chunkSize) {
  if (!m_isOpen) {
  ...
  // IMPORTANT: use a local file handle for the complete streaming operation.
  // The old implementation reused m_file. Other XTC operations may close that
  // member handle between page-table lookup and bitmap reads, which can leave
  // HalFile::impl == nullptr and trigger the HalStorage.cpp assert seen on X4.
  HalFile streamFile;
  if (!Storage.openFileForRead("XTC", m_filepath.c_str(), streamFile)) {
    return XtcError::FILE_NOT_FOUND;
  }

  // Read this page's table entry through the same local handle, so the whole
  // operation is independent of XtcParser::m_file lifecycle.
  const uint64_t entryOffset =
      m_header.pageTableOffset + static_cast<uint64_t>(pageIndex) * sizeof(PageTableEntry);
  if (!streamFile.seek64(entryOffset)) {
    streamFile.close();
    return XtcError::READ_ERROR;
  }

  PageTableEntry entry;
  const size_t entryRead = streamFile.read(reinterpret_cast<uint8_t*>(&entry), sizeof(entry));
  if (entryRead != sizeof(entry)) {
    streamFile.close();
    return XtcError::READ_ERROR;
  }

  if (!streamFile.seek64(entry.dataOffset)) {
    streamFile.close();
    return XtcError::READ_ERROR;
  }

  XtgPageHeader pageHeader;
  const size_t headerRead = streamFile.read(reinterpret_cast<uint8_t*>(&pageHeader), sizeof(pageHeader));
  const uint32_t expectedMagic = (m_bitDepth == 2) ? XTH_MAGIC : XTG_MAGIC;
  ...
  // Small fixed allocation, no 96-KB contiguous page buffer.
  ...
  while (totalRead < bitmapSize) {
    ...
    const size_t bytesRead = streamFile.read(chunk.data(), toRead);
    if (bytesRead == 0) {
      streamFile.close();
      return XtcError::READ_ERROR;
    }
