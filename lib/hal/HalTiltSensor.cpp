...
void HalTiltSensor::update(const uint8_t enabled, const uint8_t direction, const uint8_t orientation,
  ...
  const bool shouldBeAwake = (enabled != InkMODTiltPageTurn::TILT_OFF) && inReader;
  ...
  const bool forwardBack = direction == InkMODTiltPageTurnDirection::TILT_FORWARD_BACK ||
                           direction == InkMODTiltPageTurnDirection::TILT_FORWARD_BACK_INVERTED;
  const bool inverted = direction == InkMODTiltPageTurnDirection::TILT_LEFT_RIGHT_INVERTED ||
                        direction == InkMODTiltPageTurnDirection::TILT_FORWARD_BACK_INVERTED;
  float tiltAxis;
  switch (orientation) {
    case InkMODOrientation::PORTRAIT:
      tiltAxis = forwardBack ? gy : gx;
      break;
    case InkMODOrientation::INVERTED:
      tiltAxis = forwardBack ? -gy : -gx;
      break;
    case InkMODOrientation::LANDSCAPE_CW:
      tiltAxis = forwardBack ? gx : -gy;
      break;
    case InkMODOrientation::LANDSCAPE_CCW:
      tiltAxis = forwardBack ? -gx : gy;
      break;
    default:
      tiltAxis = forwardBack ? gy : gx;
