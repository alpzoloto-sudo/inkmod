...
PlatformIO pre-build script: inject git info into version defines.
  ...
  gh_release_rc: 1.1.0-rc+<5-char-hash>   (hash from $INKMOD_RC_HASH in
                                             CI, or from git locally)

All other environments set INKMOD_VERSION directly in platformio.ini.
"""
...
def get_base_version(project_dir):
    ...
    if not config.has_option('inkmod', 'version'):
        warn('No [inkmod] version in platformio.ini; base version will be "0.0.0"')
        return '0.0.0'
    return config.get('inkmod', 'version')


def get_inkmod_version(project_dir):
    config = _read_ini(project_dir)
    if not config.has_option('inkmod', 'inkmod_version'):
        warn('No [inkmod] inkmod_version in platformio.ini; falling back to version')
        return get_base_version(project_dir)
    return config.get('inkmod', 'inkmod_version')
...
def inject_version(env):
    ...
    if pioenv == 'default':
        base_version = get_inkmod_version(project_dir)
        branch = get_git_branch(project_dir)
        version_string = f'{base_version}-dev+{branch}'
        env.Append(CPPDEFINES=[('INKMOD_VERSION', f'\\"{version_string}\\"')])
        print(f'inkMOD build version: {version_string}')

    elif pioenv == 'debug':
        branch = get_git_branch(project_dir)
        short_hash = get_git_short_hash(project_dir)
        ci_version = get_inkmod_version(project_dir)
        suffix = f'-{branch}+{short_hash}'
        env.Append(CPPDEFINES=[
            ('INKMOD_VERSION', f'\\"{ci_version}{suffix}\\"'),
            ('INKMOD_BUILD_ENV', '\\"debug\\"'),
            'INKMOD_SHOW_SLEEP_BUILD_INFO',
        ])
        print(f'inkMOD test build version: {ci_version}{suffix}')
        ...
        # CI passes INKMOD_RC_HASH as an env var; locally we derive it from git.
        short_hash = os.environ.get('INKMOD_RC_HASH') or get_git_short_hash(project_dir)
        ci_version = get_inkmod_version(project_dir)
        rc_suffix = f'-rc+{short_hash}'
        env.Append(CPPDEFINES=[
            ('INKMOD_VERSION', f'\\"{ci_version}{rc_suffix}\\"'),
        ])
        print(f'inkMOD RC build version: {ci_version}{rc_suffix}')
