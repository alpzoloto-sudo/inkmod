...
# Runs after patch_ota_diag_v2.py and extends STAGE=2 diagnostics.
h = hdr.read_text(encoding='utf-8')
if 'lastSocketErrno' not in h:
    h = h.replace('  int lastOtaStage = 0;\n',
                  '  int lastOtaStage = 0;\n'
    ...
    h = h.replace('  int getLastOtaStage() const { return lastOtaStage; }\n',
...
if reset_old not in s:
    raise RuntimeError('v3 diagnostic reset block not found')
...
if begin_old not in s:
    raise RuntimeError('v3 esp_https_ota_begin block not found')
s = s.replace(begin_old, begin_new, 1)
src.write_text(s, encoding='utf-8')

# Error page: socket/TLS/heap after DL line.
a = act.read_text(encoding='utf-8')
if 'TLS=0x%X FL=0x%X' not in a:
    ...
    a = a.replace('    int y = top - (height + metrics.verticalSpacing) * 4;\n',
                  '    int y = top - (height + metrics.verticalSpacing) * 7;\n', 1)
    ...
    if needle not in a:
        raise RuntimeError('v3 error-page bytes block not found')
