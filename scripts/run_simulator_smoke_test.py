...
def run_smoke(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="inkmod-sim-smoke-") as temp_dir_name:
        temp_root = Path(temp_dir_name)
        simulator_book_path = prepare_fs(temp_root, book)

        env = os.environ.copy()
        env["INKMOD_SIMULATOR_SMOKE_TEST"] = "1"
        env["INKMOD_SIMULATOR_SMOKE_BOOK"] = simulator_book_path
        env["INKMOD_SIMULATOR_SMOKE_PAGE_TURNS"] = str(args.page_turns)
        if args.theme:
            env["INKMOD_SIMULATOR_SMOKE_THEME"] = str(THEMES[args.theme])
        if args.headless:
