...
class RecentBooksStore {
 ...
 public:
  ...
  // Clear only the in-memory list. Used by the global cache-clear action
  // after its backing file has been removed from /.inkmod; intentionally does
  // not save an empty file back to disk.
  void clearInMemory() { recentBooks.clear(); }
