...
class SimulatorSmokeTest {
 ...
 private:
  ...
  static bool enabled() { return std::getenv("INKMOD_SIMULATOR_SMOKE_TEST") != nullptr; }

  static int pageTurnCount() {
    const char* raw = std::getenv("INKMOD_SIMULATOR_SMOKE_PAGE_TURNS");
  ...
  static void applyRequestedTheme() {
    const char* raw = std::getenv("INKMOD_SIMULATOR_SMOKE_THEME");
    if (raw == nullptr || raw[0] == '\0') {
      return;
    }

    const int theme = std::atoi(raw);
    if (theme < 0 || theme >= InkMODSettings::UI_THEME_COUNT) {
  ...
  void tickImpl() {
    ...
    switch (step) {
      case SmokeStep::Start:
        ...
        if (!InkMODSettings::verifySleepTimeoutMigrationContract()) {
          fail("Sleep timeout migration contract failed");
        }
        if (!InkMODSettings::verifySleepScreenMigrationContract()) {
      ...
      case SmokeStep::Sleep: {
        const char* bookPath = std::getenv("INKMOD_SIMULATOR_SMOKE_BOOK");
        if (bookPath == nullptr || bookPath[0] == '\0') {
          LOG_INF("SMOKE", "Skipping Reader step; INKMOD_SIMULATOR_SMOKE_BOOK is not set");
